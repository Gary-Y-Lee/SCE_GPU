...
```
for n in $(grep -v '^\s*#' /etc/hosts | grep -o 'compute-hpc-node-[0-9]\+' | sort -u); do
  echo "==== $n ===="
  ssh $n "sudo systemctl restart slurmd"
  ssh $n "sinfo"
done
```

***eth100, eth101 interface instead of eth0 and eth1***

According to Linux network interface naming rules, in large-scale systems — such as GPU servers or environments with many NIC ports or high-speed networks — interface names may be generated by combining the PCI bus number, slot number, and function number.
As a result, network interfaces identifier can appear as eth100 and eth101 instead of the traditional eth0 and eth1.

You can check the network interface identifier with following command.
